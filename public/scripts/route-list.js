        ...
        if(objJson == ''){
         ...
         let changePage = function(page) {
             ...
             for(let i = (page -1) * records_per_page; i < (page * records_per_page) && i < objJson.length; i++) {
                  ...
                  let waypointsLength = objJson[i].waypoints.length;

                  const tr = document.createElement("tr");

                 const content = `
                 <td id="td-${i}"><b>${objJson[i].completed}</td>
                 <td>${objJson[i].waypoints[0]}</td>
                 <td>${objJson[i].waypoints[waypointsLength-1]}</td>
                 <td>${objJson[i].distance}</td>
                 <td>${objJson[i].timeDuration}</td>
                 <td>${objJson[i].date}</td>
                 <td>${objJson[i].startTime}</td>
